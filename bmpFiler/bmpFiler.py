...
calc_raw_data = lambda w, h, b:b*w*h+(w%4)*h
...
def generate_bmp_header(raw_data, color_mode):
    offset = 54
    if(color_mode == COLOR_MODE_RGBA or color_mode == COLOR_MODE_BGRA):
        offset = 122
...
def generate_dib_header(color_mode, shape, raw_data):
    ...
    if  (color_mode == COLOR_MODE_RGB or color_mode == COLOR_MODE_BGR):
        dim_bytes = (40).to_bytes(4, byteorder="little")
        bits = b'\x18\x00'
        bi_rgb = b'\x00\x00\x00\x00'
    elif(color_mode == COLOR_MODE_RGBA or color_mode == COLOR_MODE_BGRA):
...
def make_file(color_grid, color_mode=1, path="", name="test"):
    ...
    raw_data = 0
    if(color_mode == 1 or color_mode == 2):
        raw_data = calc_raw_data(w, h, 3)
    elif(color_mode == 3 or color_mode == 4):
        raw_data = calc_raw_data(w, h, 4)


    bmp_header = generate_bmp_header(raw_data, color_mode)

    dib_header = generate_dib_header(color_mode, (w, h), raw_data)
        
    bitmap_data = create_bitmap_data(color_grid, w%4 if(color_mode == 1 or color_mode == 2) else 0, color_mode)
